...
dsName = 'EmoDB';
[allFeaturesInit, allLabels, k, nKIt, nTest, nIt, sigma] = ...
    importData(dataPath, dsName);
nSamples = numel(allLabels);

%% Determine top class
uVal = unique(allLabels);
[numMaxLabel maxNum] = max(histc(allLabels,uVal));
maxLabel = uVal(maxNum);
zeroedLabelsInit = double(allLabels==maxLabel);
...
for itOn = 1:nIt
    ...
    zeroedLabels = zeroedLabelsInit(rp);
    T_features = allFeatures(1:nTest,:);
    T_labels = zeroedLabels(1:nTest);
    Tr_features = allFeatures(nTest+1:end,:);
    Tr_labels = zeroedLabels(nTest+1:end,:);
